...
void VizStressMinimization::fixXCoordinates(bool fix) {
	m_fixXCoords = fix;
}

void VizStressMinimization::fixYCoordinates(bool fix) {
	m_fixYCoords = fix;
}

void VizStressMinimization::hasInitialLayout(bool hasInitialLayout) {
	m_hasInitialLayout = hasInitialLayout;
}

void VizStressMinimization::layoutComponentsSeparately(bool separate) {
	m_componentLayout = separate;
}

void VizStressMinimization::setEdgeCosts(double edgeCosts) {
	m_edgeCosts = (edgeCosts > 0) ? edgeCosts : 100;
}

void VizStressMinimization::setIterations(int numberOfIterations) {
	m_numberOfIterations = (numberOfIterations > 0) ? numberOfIterations : 100;
}

void VizStressMinimization::convergenceCriterion(TerminationCriterion criterion) {
	m_terminationCriterion = criterion;
}

void VizStressMinimization::useEdgeCostsAttribute(bool useEdgeCostsAttribute) {
