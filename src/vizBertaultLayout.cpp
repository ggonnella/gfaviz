/** \file
 * \brief Declaration of class VizBertaultLayout.
...
VizBertaultLayout::VizBertaultLayout()
{
	req_length=0;
	iter_no=0;
		impred=false;
...
VizBertaultLayout::VizBertaultLayout(double length, int number)
{
	req_length=length;
	iter_no=number;
		impred=false;
...
VizBertaultLayout::VizBertaultLayout(int number)
{
	req_length=0;
	iter_no=number;
		impred=false;
...
void VizBertaultLayout::call(GraphAttributes &AG)
{
...
void VizBertaultLayout::f_Node_Repulsive(node *v,node *j, GraphAttributes &AG)
{
  if (v < j)
    return;
    
  double dx = AG.x(*v)-AG.x(*j);
  double dy = AG.y(*v)-AG.y(*j);
  double distsquared = dx*dx+dy*dy;
	//double dist=sqrt((AG.x(*v)-AG.x(*j))*(AG.x(*v)-AG.x(*j))+(AG.y(*v)-AG.y(*j))*(AG.y(*v)-AG.y(*j)));
  
  if (distsquared > req_length*req_length*3)
    return;

  //double dist = sqrt(distsquared);
  double mult = req_length/sqrt(distsquared);
  mult *= mult;
  
  double fx = mult*(AG.x(*v)-AG.x(*j));
  double fy = mult*(AG.y(*v)-AG.y(*j));
  
	//(F_x)[*v]+=mult*(AG.x(*v)-AG.x(*j));
	//(F_y)[*v]+=mult*(AG.y(*v)-AG.y(*j));
  (F_x)[*v]+=fx;
  (F_y)[*v]+=fy;
  (F_x)[*j]-=fx;
  (F_y)[*j]-=fy;
...
void VizBertaultLayout::f_Node_Attractive(node *v,node *j, GraphAttributes &AG)
{
	double dist=sqrt((AG.x(*v)-AG.x(*j))*(AG.x(*v)-AG.x(*j))+(AG.y(*v)-AG.y(*j))*(AG.y(*v)-AG.y(*j)));
...
void VizBertaultLayout::compute_I(node *v,edge *e, GraphAttributes &AG)
{
	node a=(*e)->source();
	node b=(*e)->target();
...
bool VizBertaultLayout::i_On_Edge(edge *e, GraphAttributes &AG)
{
...
void VizBertaultLayout::f_Edge(node *v,edge *e, GraphAttributes &AG)
{
...
void VizBertaultLayout::r_Calc_On_Edge(node *v, edge *e, GraphAttributes &AG)
{
	node a=(*e)->source();
	node b=(*e)->target();
	int s=0;
...
void VizBertaultLayout::r_Calc_Outside_Edge(node *v, edge *e, GraphAttributes &AG)
{
	node a=(*e)->source();
	node b=(*e)->target();
...
void VizBertaultLayout::move(node *v, GraphAttributes &AG)
{
	int s=0;
	double x_diff=(F_x)[*v];
	double y_diff=(F_y)[*v];
...
void VizBertaultLayout::initPositions(GraphAttributes &AG, char c)
{
...
void VizBertaultLayout::preprocess(GraphAttributes &AG)
{
	const Graph &G = AG.constGraph();
...
void VizBertaultLayout::labelling(GraphAttributes &AG)
{
...
void VizBertaultLayout::crossingPlanarize(GraphAttributes &AG)
{
...
int VizBertaultLayout::insert(CCElement *new1,CCElement *element,GraphAttributes &PAG,PlanRep &PG)
{
...
int VizBertaultLayout::contained(CCElement* new1,CCElement* element,GraphAttributes &PAG,PlanRep &PG)
{
#if 0
...
void VizBertaultLayout::compute(CCElement* element, PlanRep &PG, GraphAttributes &AG1, GraphCopy &G1)
{
	int num = element->num;
	PG.initCC(num);
	ConstCombinatorialEmbedding E(PG);
...
int VizBertaultLayout::edgeCrossings(GraphAttributes &AG)
{
	const Graph &G = AG.constGraph();
	int crossings = 0;
	for (edge e : G.edges)
...
double VizBertaultLayout::edgelength(GraphAttributes &AG)
{
	EdgeArray<double> el;
	const Graph &G = AG.constGraph();
	el.init(G);
...
double VizBertaultLayout::nodeDistribution(GraphAttributes &AG)
{
	const Graph &G = AG.constGraph();
